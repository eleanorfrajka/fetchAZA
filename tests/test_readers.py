...
def test_parse_column_name():
    ...
    assert readers.parse_column_name("UsedPercentage %") == ("UsedPercentage", "percent")
...
def test_standardise_dataset_constant_and_datetime():
    ds = xr.Dataset({
        "UID": ("time", ["007217"] * 3),
        "Node Ref": ("time", [""] * 3),
        "Record Time": ("time", ["2025/02/18 15:14:43", "2025/02/18 15:14:22", "2025/02/18 15:14:23"]),
...
"""
    ...
    try:
        ...
        expected_vars = ["Record Time", "Retrieval Time", "Node Ref", "UID", "UsedPercentage", "Volts"]
        for var in expected_vars:
            assert var in ds.data_vars, f"Expected variable {var} not found in the BAT dataset."
    finally:
...
"""
    ...
    try:
        ...
        assert "Transfer Temperature" in ds.data_vars, "Transfer Temperature variable missing."

        # Check that 'Transfer Temperature' has the attribute 'units'.
        assert "units" in ds["Transfer Temperature"].attrs, "Transfer Temperature variable is missing the 'units' attribute."
